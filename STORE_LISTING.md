...
```text
YouTubeの一時停止・再開・シーク時に音量を短くフェードし、急な音の切れや立ち上がりをやわらげます。
```

### 詳細な説明（16,000文字以内）

```text
YouTubeの一時停止・再開・シーク時に、音量を短くフェードさせるChrome拡張機能です。
...
本拡張機能は、個人情報、YouTubeアカウント情報、視聴履歴、動画タイトル、動画URL、Cookie、音声・映像コンテンツを収集しません。
...
・YouTubeサイトへのアクセス
　YouTubeのメインプレイヤーに音量フェードを適用するために必要です。他のWebサイトでは動作しません。

【使い方】

1. 拡張機能をインストールします。
2. YouTubeのタブを再読み込みします。
3. 通常どおり動画を再生・一時停止・シークします。
4. 必要に応じて、ツールバーの拡張機能アイコンから各機能とフェード時間を調整します。

本拡張機能はYouTubeまたはGoogleの公式製品ではなく、独立した拡張機能です。
```
...
```text
YouTubeの再生画面と、拡張機能のポップアップを同時に表示。
ポップアップ内の「すべて有効」「フェードアウト」「フェードイン」が読める状態。
キャプション: 一時停止・再開・シーク時の急な音量変化をやわらげます
```
...
表示文字: Pause Resume Audio Fade
補助文字: YouTubeの音を、やさしく止めて、やさしく戻す。
```
...
```text
YouTubeのメインプレイヤーで、一時停止・再生再開・シーク時の音量を短時間フェードさせ、急な音量変化を緩和すること。
```
...
```text
YouTubeのメイン動画プレイヤーの一時停止・再生再開・シークを検知し、音量フェードを適用するために必要です。ページ内容や視聴履歴の収集には使用しません。
```

#### `https://m.youtube.com/*`

```text
YouTubeモバイルWeb版のメイン動画プレイヤーへ同じ音量フェード機能を適用するために必要です。
```

#### `https://music.youtube.com/*`

```text
YouTube Music Web版のメインプレイヤーへ同じ音量フェード機能を適用するために必要です。
```
...
```text
1. 拡張機能をインストールします。
2. https://www.youtube.com/ で任意の動画を開き、タブを再読み込みします。
...
音声を確認できる環境が必要です。YouTubeへのログインは不要です。
```

審査メモ:

```text
本拡張は、YouTubeのメインプレイヤーの音量遷移だけを変更します。ネットワーク通信、外部コード、広告、解析、アカウント情報・視聴履歴・動画情報の収集はありません。storage権限は、オン／オフとフェード時間の端末内保存にのみ使用します。
```

## 7. 提出前チェック

- [ ] `youtube-pause-resume-fade-1.0.0.zip` のルート直下に `manifest.json` がある
...
Developer Dashboard の「言語」で各ロケールを追加し、以下をコピーして入力します。UIは拡張パッケージ内の `_locales` で自動的に切り替わります。
...
```text
Softly fade YouTube audio on pause, resume, and seek so sudden volume changes are easier on your ears.
```

詳細な説明 / Detailed description:

```text
A Chrome extension that briefly fades YouTube audio when you pause, resume, or seek.
...
This extension does not collect personal information, YouTube account data, watch history, video titles, video URLs, cookies, or audio/video content.
...
- Access to YouTube sites
  Required to apply the audio fade to the main YouTube player. It does not run on other websites.

[How to use]

1. Install the extension.
2. Reload your YouTube tab.
3. Play, pause, and seek videos as usual.
4. Adjust each feature and the fade durations from the toolbar icon if you like.

This extension is an independent project and is not an official YouTube or Google product.
```
...
- Fade out on pause
- Fade in on resume
- Fade in when seeking during playback
- Per-feature on/off
- Master on/off for all features
...
詳細な説明:「1. Store Listing」の日本語版本文（既存）をそのまま使用します。
...
```text
在 YouTube 暂停、恢复或跳转时对音量做短暂淡入淡出，减轻突然的音量变化，让听感更柔和。
```

详细说明:

```text
这是一款 Chrome 扩展，在 YouTube 暂停、恢复或跳转时对音量进行短暂的淡入淡出。
...
本扩展不收集个人信息、YouTube 账号信息、观看记录、视频标题、视频网址、Cookie，或音视频内容。
...
- 访问 YouTube 网站
  用于对 YouTube 主播放器应用音量淡变。它不会在其他网站运行。
...
本扩展为独立项目,并非 YouTube 或 Google 的官方产品。
```
...
```text
在 YouTube 暫停、恢復或跳轉時對音量做短暫淡入淡出，減輕突然的音量變化，讓聽感更柔和。
```

詳細說明:

```text
這是一款 Chrome 擴充功能，在 YouTube 暫停、恢復或跳轉時對音量進行短暫的淡入淡出。
...
本擴充功能不會收集個人資訊、YouTube 帳號資訊、觀看紀錄、影片標題、影片網址、Cookie,或音訊/影片內容。
...
- 存取 YouTube 網站
  用於對 YouTube 主播放器套用音量淡變。它不會在其他網站執行。
...
本擴充功能為獨立專案,並非 YouTube 或 Google 的官方產品。
```
