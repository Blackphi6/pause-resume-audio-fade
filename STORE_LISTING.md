...
**1.6.0 → 1.6.1（今回）:** 誤って別の拡張機能（Video Speed Controller Plus）と勘違いして、オンページのクイック設定メニュー機能を 1.6.0 として公開してしまったため、コードを 1.5.3 相当（クイックメニューなし）に戻し、バージョン番号だけ 1.6.1 に上げて再提出します。Chrome ウェブストアはバージョンを後戻りできないため、番号は 1.6.1 に進めつつ中身は 1.5.3 と同じにする形です。
...
|--------------------|--------------------|----------|
| パッケージ（左メニュー） | （ファイル） | リポジトリ直下の `youtube-pause-resume-fade-1.6.1.zip` |
...
- [ ] `youtube-pause-resume-fade-1.6.1.zip` のルート直下に `manifest.json` がある
