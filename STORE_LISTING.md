...
**1.6.0 → 1.6.1（誤公開の巻き戻し）:** 誤って別の拡張機能（Video Speed Controller Plus）と勘違いして、オンページのクイック設定メニュー機能を 1.6.0 として公開してしまったため、コードを 1.5.3 相当（クイックメニューなし）に戻し、バージョン番号だけ 1.6.1 に上げて再提出した。Chrome ウェブストアはバージョンを後戻りできないため、番号だけ進めて中身は 1.5.3 と同じにした。

**1.6.2（今回）:** モバイル版 YouTube で、一時停止してから再生を再開すると音が出なくなることがある不具合を修正。説明文の【今回の更新】もこの内容に更新済み。

| ダッシュボードの欄 | このファイルの場所 | 貼る内容 |
|--------------------|--------------------|----------|
| パッケージ（左メニュー） | （ファイル） | リポジトリ直下の `youtube-pause-resume-fade-1.6.2.zip` |
| パッケージの概要 | [`STORE_PASTE.md`](STORE_PASTE.md) | 編集不可・ZIPから自動 |
...
| 単一目的・ホスト権限が必要な理由 | **「3. Privacy Practices」** | 変更不要（新しい権限は追加していない） |
...
別欄はありません。上の「詳細な説明」の【今回の更新】に、すでに同じ文章が入っています（モバイルで一時停止→再生後に音が出なくなる不具合の修正）。説明欄を枠ごと貼り直せば足ります。
