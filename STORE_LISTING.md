...
**今回（1.6.0）で変わったところ:**
- 動画ページ上に小さなボタン（クイックメニュー）を追加し、ツールバーのポップアップを開かなくても設定を変更できるようにした（コード内のみの変更。ホスト権限・単一目的の文言に変更なし）
- 説明文の【今回の更新】は「動画ページから直接設定を開けるクイックメニューを追加しました」に更新。サイト名は書きません

| ダッシュボードの欄 | このファイルの場所 | 貼る内容 |
|--------------------|--------------------|----------|
| パッケージ（左メニュー） | （ファイル） | リポジトリ直下の `youtube-pause-resume-fade-1.6.0.zip` |
...
| 単一目的・ホスト権限が必要な理由 | **「3. Privacy Practices」** | 1.6.0では変更なし（新しいホスト権限は追加していない） |
| テスト手順（左メニュー「テスト手順」） | **「6. Test Instructions」** | 変更なし |
...
### バージョン1.6.0の更新内容（説明の先頭に入れる）

別欄はありません。上の「詳細な説明」の【今回の更新】に、すでに同じ文章が入っています（動画ページから直接設定を開けるクイックメニューを追加。説明にはサイト名を書きません）。説明欄を枠ごと貼り直せば足ります。
...
- [ ] `youtube-pause-resume-fade-1.6.0.zip` のルート直下に `manifest.json` がある
