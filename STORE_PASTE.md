...
**パッケージの概要は、ここからは変えられません。** `manifest.json` の `description`（各言語の `extDescription`）から自動で入ります。概要を直すには **1.6.1 の ZIP をアップロード** してください（すでに修正済み）。
...
**編集不可。** ZIP 1.6.1 を上げると、次の文に自動で変わります。
...
**Read-only on the dashboard.** Upload ZIP 1.6.1; it becomes:
