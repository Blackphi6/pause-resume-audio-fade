...
**最終更新:** 2026-07-17（v1.0.0）

## 要約

本拡張機能は、**個人情報・視聴履歴・YouTube アカウント情報・動画 ID・Cookie・音声／映像コンテンツを収集しません。**  
開発者や第三者のサーバーへデータを送信する通信処理も**ありません。**

## 目的（単一目的）

YouTube（`www.youtube.com` / `m.youtube.com` / `music.youtube.com`）上で、一時停止・再生再開・シーク時の音量を短時間フェードし、急な音の切れ／立ち上がりを緩和します。
...
1. **音量制御** — メインプレイヤー映像に限り再生／一時停止経路をフックし、必要時のみ Web Audio の GainNode で音量を変更します。録音・解析・アップロードはしません。
...
|------|------|
| `storage` | 設定値の端末内保存 |
| YouTube 系 HTTPS コンテンツスクリプト | 当該サイトでのフェード適用 |
...
**Last updated:** 2026-07-17 (v1.0.0)

No personal data, watch history, account data, or AV content is collected or transmitted. Only on-device preference booleans and fade durations are stored in `chrome.storage.local` (`incognito: split`). Legacy sync keys are migrated once then cleared. Volume fades run only on the main YouTube player via optional GainNode (no capture). Settings cross the isolated→page boundary via a `chrome.runtime.id`-tokenized `CustomEvent`, not a public `postMessage` channel.
